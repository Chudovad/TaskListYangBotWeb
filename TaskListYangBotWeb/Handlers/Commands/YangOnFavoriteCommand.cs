        ...
        public override async Task ExecuteAsync(Update update)
        {
            if (CommandStatus.commandStatus[update.Message.Chat.Id] == false)
            {
                ...
                List<string?> listFavoriteTasks = _favoriteTaskRepository
                    ...
                    .ToList();
                await AutomaticTaskPickupService.StartYangONCommand(update.Message.Chat.Id, true, _telegramBotClient, tokenYang, typeSorting, listFavoriteTasks, CommandNames.YangOnFavoriteCommand);
            }
            else
            {
